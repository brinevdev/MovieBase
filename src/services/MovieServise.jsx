...
export default class MovieService {
    ...
    getMovie = async (id) => {
        const res = await fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=${this._APIKEY}&language=ru-RU`)
        const movie = await res.json()
        return movie;
    } 

    getActors = async (movieId) => {
        const res = await fetch(`https://api.themoviedb.org/3/movie/${movieId}/credits?api_key=${this._APIKEY}&language=ru-RU`);
